...
exports.mobileSMS = async function (phoneno, content, detail= {}) {
  try {
    if(config.sendSMS) {
      if(config.sendSMS.enable) {
        ...
        if(config.sendSMS.sectionStatus && section) {
          if(config.sendSMS.sectionStatus[section] === "Disable") {
            return false;
          }
        }

        content = content.replace("[ Exchange ]", "[ "+config.siteName+" ]")
        if(config.sendSMS.name == "twilio") {
