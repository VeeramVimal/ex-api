            ...
            if (users.status) {
                ...
                if (coinAddress.length > 0) {
                    for (i = 0; i < coinAddress.length; i++) {
                        addressArr[coinAddress[i].currencyname] = { address: coinAddress[i].address, tag: coinAddress[i].tag }
                    }
                }
                let newarr = [];
                if (currency.length > 0) {
                    currency.forEach(element => {
                        ...
                        const addressObj = typeof addressArr[curName] == 'object' ? addressArr[curName] : { address: '-', tag: '-' };
