...
const currencyController = {
    ...
    async getCurrencyfilter(req, res) {
        try {
            ...
            res.json({
                "status": currency.status, "getCurrencyTblDetails": currency.msg,
                "total": currencycount
            });
        } catch (e) {
    ...
    async updateCurrency(req, res) {
        ...
        if (!getCurrency.status) {
            ...
            if (!getCurrency1.status) {
                await query_helper.updateData(Currency, "many",
                    { currencySymbol: data.currencySymbol },
                    {
                        transferEnable: data.transferEnable,
                        stakingTransferEnable: data.stakingTransferEnable,
                        gamePredictionTransferEnable: data.gamePredictionTransferEnable,
                        cryptoLoanTransferEnable: data.cryptoLoanTransferEnable,
                        simpleEarnTransferEnable: data.simpleEarnTransferEnable
                    });
