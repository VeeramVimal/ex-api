...
const p2pController = {
                        ...
                        if (reqBody.OTPCode != "" && reqBody.OTPCode != undefined && reqBody.OTPCode != "undefined") {
                            ...
                            if (checkTxn.status) {
                                ...
                                if (orderData.status) {
                                    ...
                                    if (orderData.orderMode == "Offline") {
                                       return res.json({ status: false, message: "Now this order is offline.",type: 0 });
                                    }
    ...
    async changeMode(req,res){
        try {
            let data = req.body;
            let orderStatus = await query_helper.findoneData(P2POrder, { _id: mongoose.Types.ObjectId(data.modeId), status: 1 }, {});
            if (orderStatus.status) {
                orderStatus = orderStatus.msg;
                let status = 'Online';
                orderStatus.orderMode == 'Online' ? status = 'Offline' : status = 'Online';
                await query_helper.updateData(P2POrder, "one", { _id: mongoose.Types.ObjectId(orderStatus._id) }, { orderMode : status});
                res.json({ status: true, message: orderStatus.orderMode == 'Online' ?  "Offline Mode Disabled" : "Online Mode Enabled" })
            } else {
                res.json({ status: false, message: "Invalid order" });
            }
        } catch(err) {}
