...
exports.socChannelUpd = async function (passData = {}) {
  ...
  let args_s = [];
  args_s.push(`trade.${channelNameUpd}`);
  let subscribe = { op: "subscribe", args: args_s };
  socket.send(JSON.stringify(subscribe));

  let args_sob = [];
  args_sob.push(`orderBookL2_25.${channelNameUpd}`);
  let subscribeOB = { op: "subscribe", args: args_sob };
  socket_OB.send(JSON.stringify(subscribeOB));
};

exports.connectWS = async function (passData = {}) {
  await connectWS_init_RT(passData);
  await connectWS_init_OB(passData);
};
      ...
      if (pair && Object.keys(pair).length > 0) {
        if (pair.autoOrderExecute === 1 || pair.autoOrderExecute === "1") {
          if((respData && respData.type) && (respData.type == 'snapshot' || respData.type == 'delta')) {
            ...
            // Sell
            let sellOrder = [], bybitSellOrder = bybitOB.filter(el => el.side == 'Sell');
            ...
            sellOrder.forEach(async (pairElement, key) => {
              let priVal = +pairElement._id;
              let sizeVal = +pairElement.amount;
            ...
            // Buy
            let buyOrder = [], bybitBuyOrder = bybitOB.filter(el => el.side == 'Buy');
            ...
            buyOrder.forEach(async (pairElement, key) => {
              let priVal = +pairElement._id;
              let sizeVal = +pairElement.amount;
              if (priceChange.changeValue > 0) {
                priVal = priceChange.changePer * priVal;
            ...
            if (orderwith == -1 || timeLimit == false) {
